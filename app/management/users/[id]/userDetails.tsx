        ...
        <form className="mt-6 border-t border-zinc-200 dark:border-zinc-500" onSubmit={(e) => handleSubmit(e)} >
          <div className="divide-y divide-zinc-200 dark:divide-zinc-500">
            ...
            <div className="px-4 py-6 flex flex-row gap-x-4 sm:px-1 items-center">
              <div className="text-lg leading-6 text-center text-zinc-500 dark:text-zinc-500 w-32">ID</div>
              <TextInput disabled={true} className='outline-0 w-1/3 h-1/2' name='id' placeholder={User.id} type='text' />
            </div>
                  ...
                  <>
                    {role === 'ADMIN' ? (
                      <>
                        ...
                        <InvertedSubmitButton onClick={() => setIsModalOpen(true)} type="button">Delete User</InvertedSubmitButton>
                      </>
                    ) : (
                      <SubmitButton onClick={handleEditToggle} type="button">Edit</SubmitButton>)}
                  </>
                ) : (
                  <SubmitButton onClick={() => handlePasswordReset(User.email)} type="button">Reset User Password</SubmitButton>
                )}
              </>
            )}
          </div>
