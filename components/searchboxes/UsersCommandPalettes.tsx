...
export function UsersSearchPalette<T>({
  ...
  const resources = data.map((user: Resource) => {
    return {
      id: user.id,
      resource: user.resource,
      resourceType: user.resourceType,
      resourceLocation: user.resourceLocation,
      href: user.href,
    };
  }
  );
                ...
                {rawQuery === '?' && (
                  <div className="px-6 py-14 text-center text-sm sm:px-14">
                    ...
                    <p className="mt-2 text-zinc-500">
                      Use this tool to quickly search for previous users on the platform. You can also
...
export function AdminUserSearchPalette<T>({
  data,
  onClose,
}: UsersSearchPaletteProps<T>): JSX.Element {
  const [open, setOpen] = useState(true);
  ...
  const resources = data.map((user: AdminResource) => {
    return {
      id: user.id,
      user: user.user,
      resource: user.resource,
